#!/usr/bin/env python3
"""Validate cartridge-only voice and static layer overlay entries/payloads."""
...
BLOCK_SIZE = 0x0400
# APS-053 T2: entries 3-6, immediately after the two voice entries. Payloads
# are generated by scripts/generate-static-layer.py.
OVERLAY_GROUPS = ("stage1", "stage2", "stage3", "title")
...
def main():
    ...
    overlay_entries = []
    overlay_payloads = []
    previous = game_over_entry
    for index, group in enumerate(OVERLAY_GROUPS):
        overlay_entry = entry(rom, 3 + index)
        if overlay_entry["flags"] != 0 or overlay_entry["destination"] != 0:
            raise SystemExit("invalid %s overlay directory entry" % group)
        if overlay_entry["cartridge_offset"] != (
            previous["cartridge_offset"] + previous["length"]
        ):
            raise SystemExit(
                "%s overlay is not contiguous after the previous cart entry" %
                group
            )
        overlay_path = Path("assets/overlay/%s.bin" % group)
        overlay_bytes = overlay_path.read_bytes()
        overlay_start = LNX_HEADER_SIZE + overlay_entry["cartridge_offset"]
        overlay_payload = rom[overlay_start:overlay_start + overlay_entry["length"]]
        if (overlay_entry["length"] != len(overlay_bytes) or
                overlay_payload != overlay_bytes):
            raise SystemExit(
                "%s overlay payload does not match assets/overlay/%s.bin" %
                (group, group)
            )
        overlay_entries.append(overlay_entry)
        overlay_payloads.append(overlay_payload)
        previous = overlay_entry

    last_start = LNX_HEADER_SIZE + overlay_entries[-1]["cartridge_offset"]
    if last_start + len(overlay_payloads[-1]) != len(rom):
        raise SystemExit("unexpected bytes follow the last overlay payload")

    for name, directory_entry, payload in (
        ("title", title_entry, title_payload),
        ("game-over", game_over_entry, game_over_payload),
    ) + tuple(zip(OVERLAY_GROUPS, overlay_entries, overlay_payloads)):
        print(
