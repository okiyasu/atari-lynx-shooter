...
- 状態: **実装・host/strict・release回帰・Gearlynx GUI/headless PASS**。開始HEAD=`a0fe46d7b99296667c4e1c20a6fe69d41878b14f`、検証時終了HEAD=`a0fe46d7b99296667c4e1c20a6fe69d41878b14f`。v007〜v010の未コミットAPS-056差分を保全し、v011変更を同一作業ツリーへ適用。commit/push前の最終状態。
- 実装: `src/main.c`の`MOVABLE_SET_HPOS/VPOS`を`SCB_RENONE` raw byte offset `7/9`への直接storeへ変更。v010で確認した`&(scb)->hpos/vpos`のcc65 2.19誤生成（SCB base offset 0へのstore、SPRCTL0上書き）を回避。active敵弾はHUD上端未満で`SKIP`でもGameStateのhpos/vposをstoreし、slot3の座標証跡を保持。SCB chain、draw order、collision、公開API、productionデータ形式は変更なし。
- 必須SCB値（GUI/headless同値）: GameState slot0=`[100,40,1,0,0]`、slot3=`[80,9,1,0,0]`。実`tgi_ioctl` chain slot0=`SPRCTL0=0x05, SPRCTL1=0x08, hpos=100, vpos=40`、slot3=`SPRCTL0=0x05, SPRCTL1=0x0C(SKIP), hpos=80, vpos=9`。trace=`active_count=2, visible_count=1, penpal[0]=6, marker=0xA5/0x5A, submit_count=1`。
- framebuffer: GUI/headlessともpost-submit `vidbas==dispadr`、slot0 `(100,40)`のscreenshot/vidbas/dispadr全probeでenemy-bullet palette index 6色を検出。証跡=`evidence/APS-056/scb-trace-v011.json`およびGUI/headless各PNG。
- artifacts: release `/Users/mammycloud-m4/Documents/develop-m4/atari-lynx-shooter/dist/asteroid-patrol.lnx`=`61131 bytes`、SHA-256=`8e162a18f3dd70004fd7832efcb581da6cd86003d791e6cf14d0c812eab8bd43`、map MAIN使用=`46361B`/余剰=`415B`。diagnostic `/Users/mammycloud-m4/Documents/develop-m4/atari-lynx-shooter/dist/asteroid-patrol-aps056-diagnostic.lnx`=`60151 bytes`、SHA-256=`94cf7f82efa0a78e00f88adfd4138e8c469a76d7879de66c2add3748f97f5fe7`、map MAIN使用=`45661B`/余剰=`1451B`。両LNX=`magic=LYNX version=1 bank0_page=1024 bank1_page=0`、`GAME_VERSION_STRING=0.53.19`。
- 検証: `make aps056-diagnostic-host`（0、28 checks）、`make test smoke-host lint`（0、stage155/game652/sound351/IMA14949/sprite197/APS-055 14/APS-056 28/smoke19、strict cc65、shell lint）、`make clean && ./scripts/verify.sh`（0、release LNX/voice/cart全PASS）、`make aps056-diagnostic-gearlynx`（0、LNX PASS、headless PASS、GUI PASS）、`python3 -m py_compile scripts/verify-aps056-diagnostic-gearlynx.py`（0）、`git diff --check`（0）。
- 設計差分/未確認: なし。実機Suzy/LCD/OPT入力は未確認（ブリーフ指定どおり実機操作なし）。

### APS-056 v010 v009直接計測の原因付き再検証（2026-08-23）

- 状態: **原因確定・検証器誤PASS修正・host/strict検証完了、対策未着手**。開始HEAD=`a0fe46d7b99296667c4e1c20a6fe69d41878b14f`、終了HEAD=`a0fe46d7b99296667c4e1c20a6fe69d41878b14f`。commit/push/実機投入なし。v009 evidence/PNGは保全し、v010へ期待SCB field checksを追加。
- 検証器修正: `scripts/verify-aps056-diagnostic-gearlynx.py`をv010化し、`SCB_RENONE` expected layout/fixture値を`checks`へ追加。slot0 expected=`SPRCTL0=0x05`、`SPRCTL1=0x08`、`hpos=100`、`vpos=40`、slot3 expected=`SPRCTL0=0x05`、`SPRCTL1=0x0C(SKIP)`、`hpos=80`、`vpos=9`。GUI/headlessのいずれかでfalseならattempt=`FAIL`、全体も`FAIL`（v009の`interpretation`内falseを無視した全体`PASS`を解消）。Makefile出力を`evidence/APS-056/scb-trace-v010.json`へ変更。
- layout突合: cc65 `.cache/cc65-2.19/source/include/_suzy.h:176-184`の`SCB_RENONE`は`sprctl0=0`、`sprctl1=1`、`next=3`、`data=5`、`hpos=7`、`vpos=9`、`sizeof=11`。生成assemblyのstatic DATA initializerはslot0へ`$05,$0C,$20,next,data,$0000,$0000`を出力し、初期値・chain pointer・data pointerは正常。
- 原因: `src/main.c:522-535`の`MOVABLE_SET_HPOS/VPOS`（`&scb->hpos/vpos`のlow-byte store）をcc65へ生成させた`src/main.c:1149-1157`のenemy-bullet branchで、実行時のstoreがSCB offset `7/9`ではなくSCB base offset `0`へ到達。slot0 raw=`[0x28,0x08,0x20,next,data,0,0,0,0,0x05,0x0C,0x20,...]`となり、`0x28`はfixture `vpos=40`による`SPRCTL0`上書き、hpos/vposは0のまま。slot3はSKIP branchのためstatic `SPRCTL0=0x05`を保持し、座標だけ0のまま。`_tgi_ioctl` breakpoint境界でSuzy投入前に観測しており、static initializer不良・chain pointer誤認・Suzy/LCD固有原因ではない。
- 直接証跡（GUI/headless同値）: GameState slot0=`[100,40,1,0,0]`、slot3=`[80,9,1,0,0]`、SCB slot0 address=`0xA6E6`、slot0=`SPRCTL0=0x28, SPRCTL1=0x08, hpos=0, vpos=0, data=0x8CF5, next=0xA6F1`、slot3=`SPRCTL0=0x05, SPRCTL1=0x0C, hpos=0, vpos=0, data=0x8CF5, next=0xA712`。traceは`active_count=2`、`visible_count=1`、penpal=`6`、marker=`0xA5/0x5A`、submit=`1`。v010 `evidence/APS-056/scb-trace-v010.json`の両attempt status=`FAIL`、false checksを保存。
- framebuffer結果: GUI/headlessともpost-submit `vidbas==dispadr`、screenshot SHA-256=`1f660d625af78a3127c8a1e1db864395bbe53d32da3bb1494c55e34b6c14dfdd`、fixture slot0 `(100,40)` probeは全経路黒、palette index 6色なし。v009 PNG結果を保持し、v010 PNGを追加。
- 診断ROM: `/Users/mammycloud-m4/Documents/develop-m4/atari-lynx-shooter/dist/asteroid-patrol-aps056-diagnostic.lnx`、`60144 bytes`、SHA-256=`b1d874e2b71c8e794cabe212932c27bafe2e3faf3f5b9b40fbd416237d87a2d7`、`GAME_VERSION_STRING=0.53.19`、LNX `magic=LYNX version=1 bank0_page=1024 bank1_page=0`。map Segment=`STARTUP/LOWCODE/ONCE/CODE/RODATA/DATA/BSS=109/16/27/35097/6965/946/2494`、MAIN余剰=`1458B`、trace export RAM=`0xABF6`。
- production保全: `dist/asteroid-patrol.lnx`=`61124 bytes`、SHA-256=`1d70db9c91eda9c14a92cc475382e9bc02dd7f37c2c22299f93d7e078dadd9bc`。v010でproductionソース/ROM挙動は変更なし。
- 検証: `make aps056-diagnostic-host`（0、28 checks）、`make aps056-diagnostic-gearlynx`（LNX PASS、GUI/headless readback完了、期待field不一致を正しくFAIL化・make外側でexit=`1`）、`make test smoke-host lint`（0、stage155/game652/sound351/IMA14949/sprite197/APS-055 14/APS-056 28/smoke19、strict cc65、shell lint）、`make clean && ./scripts/verify.sh`（0、release LNX/voice/cart全PASS）、`python3 -m py_compile scripts/verify-aps056-diagnostic-gearlynx.py`（0）、`git diff --check`（0）。
- 次段: `MOVABLE_SET_HPOS/VPOS`のfield store修正案を別ブリーフで裁定後に実装。v010ではproduction変更・公開API変更・既存データ変換を行わない。実機Suzy/LCD/OPT入力は未確認。

### APS-056 v009 Gearlynx debugger/MCP直接計測（2026-08-23）

- 状態: **実装・host/strict clean検証・GUI/headless Gearlynx readback・framebuffer/screenshot証跡完了**。開始HEAD=`a0fe46d7b99296667c4e1c20a6fe69d41878b14f`、終了HEAD=`a0fe46d7b99296667c4e1c20a6fe69d41878b14f`。commit/push/実機投入なし。v007/v008ブリーフ、v008既知差分、`evidence/APS-056/scb-trace-v008.json`を保全。
- 計測器: `APS056_DIAGNOSTIC`限定のtrace APIを混在引数からSCB raw pointer読込へ変更し、cc65上のSCBフィールドをそのまま固定幅RAMへ保存。verifierはAPS-055後のcc65 `GameState`配置（enemy bullets=`+69`、stage=`+212`、phase=`+213`）へ補正し、TITLE到達を前提にせず`_game_display_sync_complete`でfixture注入。`_tgi_ioctl`の実chain 45 entryから敵弾16 slotを抽出し、post-submit syncでSCB/vidbas/dispadr/screenshotを取得。
- 変更ファイル: `Makefile`、`include/game.h`、`src/game.c`、`src/main.c`、`tests/test_aps056_diagnostic.c`、`scripts/verify-aps056-diagnostic-gearlynx.py`、本記録、`evidence/APS-056/scb-trace-v009.json`およびGUI/headlessの各PNG。production `APS056_DIAGNOSTIC`未定義経路、`include/version.h`、CLAUDE、APS-055以前の記録は変更なし。
- 直接計測結果（GUI/headless同値）: fixture GameState slot0=`x=100,y=40,active=1,vx=0,vy=0`、slot3=`x=80,y=9,active=1,vx=0,vy=0`。one-shot trace最終値=`active_count=2`、`visible_count=1`、enemy-bullet header `penpal[0]=6`、before/after marker=`0xA5/0x5A`、submit count=`1`。しかし実敵弾SCB slot0（address=`0xA6E6`）は`SPRCTL0=0x28`、`SPRCTL1=0x08`、`hpos=0`、`vpos=0`、`data=0x8CF5`、`next=0xA6F1`。slot3も`SPRCTL0=0x05`、`SPRCTL1=0x0C(SKIP)`、`hpos=0`、`vpos=0`、`data=0x8CF5`、`next=0xA712`。GameState active/座標からSCB slotへのコピー不一致を、Suzy投入前の`_tgi_ioctl`境界で直接取得。
- frame結果: post-submitの`vidbas`/`dispadr`は同値、screenshotも同一SHA-256=`1f660d625af78a3127c8a1e1db864395bbe53d32da3bb1494c55e34b6c14dfdd`。fixture slot0座標(100,40)の2x2 probeは全経路で黒、enemy-bullet palette index 6色の画素なし。証跡=`evidence/APS-056/scb-trace-v009.json`、PNG=`evidence/APS-056/scb-trace-v009-{headless,gui}-{screen,vidbas,dispadr}.png`。
- 診断ROM: `/Users/mammycloud-m4/Documents/develop-m4/atari-lynx-shooter/dist/asteroid-patrol-aps056-diagnostic.lnx`、`60144 bytes`、SHA-256=`b1d874e2b71c8e794cabe212932c27bafe2e3faf3f5b9b40fbd416237d87a2d7`、`GAME_VERSION_STRING=0.53.19`、LNX `magic=LYNX version=1 bank0_page=1024 bank1_page=0`。map Segment STARTUP/LOWCODE/ONCE/CODE/RODATA/DATA/BSS=`109/16/27/35097/6965/946/2494`、合計=`45654B`、CADENCE MAIN上限=`0xB808`、MAIN余剰=`1458B`、trace export RAM=`0xABF6`。
- production保全確認: `dist/asteroid-patrol.lnx`=`61124 bytes`、SHA-256=`1d70db9c91eda9c14a92cc475382e9bc02dd7f37c2c22299f93d7e078dadd9bc`。`make clean && ./scripts/verify.sh`でrelease LNX/voice/cart全PASS、production hash既存値一致。
- 検証: `make aps056-diagnostic-host`（0、28 checks）、`make test smoke-host lint`（0、stage155/game652/sound351/IMA14949/sprite197/APS-055 14/APS-056 28/smoke19、strict cc65、shell lint）、`make clean && ./scripts/verify.sh`（0、release LNX/voice/cart全PASS）、`make aps056-diagnostic-gearlynx`（0、LNX PASS、headless PASS、GUI PASS）、`python3 -m py_compile scripts/verify-aps056-diagnostic-gearlynx.py`（0）、`git diff --check`（0）。
- 判定/次候補: active保持・visible predicate通過数・SCB投入chain存在は確認済みだが、slot0の実SCB制御値/座標がfixtureと不一致で、LCD画素も未描画。原因候補を`GameState→movable SCB slot0構築/フィールドstore`へ絞り込み、Suzy/LCD固有仮説より前段のSCB生成不一致を優先調査。OPT実入力、実Lynx LCD、commit/pushは未実施。

### APS-056 v008 one-shot SCB投入境界計装ROM（2026-08-23）

- 状態: **実装・host/strict clean検証・診断ROM生成完了、Gearlynx readback前BLOCKED**。開始HEAD=`a0fe46d7b99296667c4e1c20a6fe69d41878b14f`、終了HEAD=`a0fe46d7b99296667c4e1c20a6fe69d41878b14f`。commit/pushなし。既知未追跡`.briefs/APS-056/v007.md`を保全し、依頼指定`.briefs/APS-056/v008.md`を使用。
- 実装: `APS056_DIAGNOSTIC`限定の固定長`GameAps056ScbTrace`（184 bytes）を`src/game.c`へ追加。`src/main.c:movable_scb_update()`で最初のenemy-bullet chain更新だけlatchし、GameState active総数、既存可視predicate通過数、16 slotのindex/SPRCTL0/SPRCTL1/hpos/vpos/data/next、header penpal[0]、`tgi_sprite()`前`0xA5`・後`0x5A` marker・投入回数1をlittle-endian固定幅で保存。export symbol=`_aps056_scb_trace`。
- 診断操作: 既存OPT1 logic freeze/draw継続とOPT2 rising-edge penpal 6/15切替を維持。hostでactive count=2/visible predicate=1、HUD外active slotのSKIP、16 slot境界、one-shot再取得拒否、OPT1 freeze中のtrace latch、OPT2後のpenpal 15/6を検証。
- 変更ファイル: `Makefile`、`include/game.h`、`src/game.c`、`src/main.c`、`tests/test_aps056_diagnostic.c`、`scripts/verify-aps056-diagnostic-gearlynx.py`、本記録、`evidence/APS-056/scb-trace-v008.json`。`CLAUDE.md`、production ROM経路、`include/version.h`、APS-055以前の記録は変更なし。
- 診断ROM: `/Users/mammycloud-m4/Documents/develop-m4/atari-lynx-shooter/dist/asteroid-patrol-aps056-diagnostic.lnx`、`60141 bytes`、SHA-256=`c75e7d25941de748585162a1edd2f25c37cf4515780267f14b4eb05ee21f754c`、`GAME_VERSION_STRING=0.53.19`、LNX `magic=LYNX version=1 bank0_page=1024 bank1_page=0`。map Segment STARTUP/LOWCODE/ONCE/CODE/RODATA/DATA/BSS=`109/16/27/35094/6965/946/2494`、使用=`45651B`、CADENCE MAIN上限=`0xB808`、MAIN余剰=`1461B`。trace export RAM=`0xABF3`。
- production保全確認: `dist/asteroid-patrol.lnx`=`61124 bytes`、SHA-256=`1d70db9c91eda9c14a92cc475382e9bc02dd7f37c2c22299f93d7e078dadd9bc`、release map Segment STARTUP/LOWCODE/ONCE/CODE/RODATA/DATA/BSS=`109/16/27/36081/6963/944/2214`、使用=`46354B`、MAIN上限=`0xB6B8`、MAIN余剰=`422B`。production ROMの挙動・サイズ・LNX・voice/cart証跡は既存値と一致。
- 検証: `make aps056-diagnostic-host`（0、28 checks）、`make test smoke-host lint`（0、stage155/game652/sound351/IMA14949/sprite197/APS-055 14/APS-056 28/smoke19、strict cc65、shell lint）、`make clean && ./scripts/verify.sh`（0、release LNX/voice cart全PASS）、`make dist/asteroid-patrol-aps056-diagnostic.lnx && ./scripts/inspect-lnx.sh dist/asteroid-patrol-aps056-diagnostic.lnx`（0、LNX PASS）、`python3 -m py_compile scripts/verify-aps056-diagnostic-gearlynx.py`（0）、`git diff --check`（0）。
- 未確認/停止: `make aps056-diagnostic-gearlynx`はLNX検査後、Gearlynxが安定TITLEへ到達せず終了コード2。`evidence/APS-056/scb-trace-v008.json`は`status=blocked`で、実SCB値・実機Suzy投入後・LCD最終画素・OPT実入力をPASS扱いしていない。実機投入、commit、pushは未実施。
- 設計差分/リスク: v008必須項目から機能差分なし。trace API分の診断ROM CODE/BSS増加のみ。release productionは`APS056_DIAGNOSTIC`未定義経路を保全。`include/version.h`は変更許可範囲外のため既存`0.53.19`を維持。
...
- 状態: **実装・host/strict ROM検証完了、実機確認待ち**。開始HEAD=`12096120d2a4f822f314e16fd076eb93ba21cbe2`、終了HEAD（実装commit）=`6745b07491a21f9a52ec5bab255ac6effe38d3b9`。APS-055 v001の既知未コミット診断差分（`ISSUES.md`、`Makefile`、`include/game.h`、`src/game.c`、`tests/test_aps055_diagnostic.c`、`.briefs/APS-055/`）を保全したまま実装。
...
- 状態: **実装・クリーン検証完了、実機確認待ち**。開始HEAD=`c1ab60e29d2eaae894118bd1d01c769c35c76292`、終了HEAD（実装commit）=`dd555bb8ca6d7df418d9e2857c5a61d25a2a2c25`。APS-054 v002の機能意味を維持したまま、release MAIN余剰を`353B→534B`、cadence MAIN余剰を`217B→398B`へ回復。
...
- 変更ファイル: `src/game.c`、`ISSUES.md`、`.briefs/APS-054/v003.md`。`include/game.h`、`include/version.h`、README、design documentは変更なし。
