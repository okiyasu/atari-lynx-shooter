...
cc65公式Lynxターゲット向けの小さな横スクロール2Dシューティングです。外部画像・音声素材は使わず、160x102・16色のTGI画面にコード内の固定水平ランと1bit行マスクから背景・キャラクターを描きます。Stage 1〜3を共通状態機械で進め、各通常区間の後にデータ駆動のボス戦へ入ります。Stage 1は宇宙、Stage 2は惑星上空、Stage 3は洞窟の固有背景・敵編成・ボスに加え、通常区間だけで動く環境ギミックと固有BGMを持ちます。射撃・敵撃破・自機爆発・アイテム取得・WARNING・ボス撃破・ステージクリアの7種の効果音も、コード内の自作固定シーケンスで鳴らします。
...
- サウンド: Stage 1は遅い宇宙アルペジオ、Stage 2は速い上昇飛行モチーフ、Stage 3は低音と休符の洞窟モチーフ。単一のMIKEY channel Aで、優先度付き7種SFXがBGMを一時的に上書き
